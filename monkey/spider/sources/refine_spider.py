...
class ArchivesItem(Item):
    target_item = TextField(css_select="ul.links-of-blogroll-list li.links-of-blogroll-item")
    href = AttrField(css_select="li.links-of-blogroll-item>a", attr="href")


class ArticleListItem(Item):
    # start_urls = "https://refined-x.com/"
    target_item = TextField(css_select="div.content-wrap h2.post-title")
    title = TextField(css_select="h2.post-title>a")
    href = AttrField(css_select="h2.post-title>a", attr="href")
...
class BlogSpider(Spider):
    ...
    async def parse_item(self, res):
        ...
        async for item in ArticleListItem.get_items(html=await res.text()):
            # 已经抓取的链接不再请求
            is_exist = (
                    await self.mongo_db.source_docs.find_one({"url": (start_urls + item.href)}) or {}
            )

            if not is_exist.get("html"):
                yield Request(
                    start_urls + item.href,
                    callback=self.save,
